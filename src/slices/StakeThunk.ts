...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const kageContract = new ethers.Contract(addresses[networkID].HIRO_ADDRESS as string, ierc20Abi, signer);
    ...
    try {
      approveTx = await kageContract.approve(
        addresses[networkID].HIROSTAKING_ADDRESS,
    ...
    const kageAllowance = await kageContract.allowance(address, addresses[networkID].HIROSTAKING_ADDRESS);
...
export const changeStake = createAsyncThunk(
  ...
  async ({ action, value, provider, address, networkID }: IActionValueAsyncThunk, { dispatch }) => {
    ...
    const signer = provider.getSigner();
    const kageStakingContrat = new ethers.Contract(addresses[networkID].HIROSTAKING_ADDRESS as string, kageStakingAbi, signer);
    ...
    try {
      if (action === "stake") {
        uaData.type = "stake";
        stakeTx = await kageStakingContrat.deposit(ethers.utils.parseUnits(value, "gwei"));
      } else {
        uaData.type = "unstake";
        stakeTx = await kageStakingContrat.withDraw(ethers.utils.parseUnits(value, "gwei"));
      }
